...
class NeuralNetwork:
	...
	def init_all_neurons(self):
		...
		self.z_layers: list[np.ndarray] = [np.zeros(shape=(self.structure[i], 1)) for i in range(self._L)]
		self.layers: list[np.ndarray] = [np.zeros(shape=(self.structure[i], 1)) for i in range(self._L)]
